...
function DashBoard() {
  ...
  const [gptWebContent, setgptWebContent] = useState(
    {
      ...
      "whyChooseUsReason1ShortDescription": "write here for a whyChooseUsReason1ShortDescription",
      "whyChooseUsReason2": "write here for a whyChooseUsReason2",
      "whyChooseUsReason2ShortDescription": "write here for a whyChooseUsReason 2 Short Description",
      "whyChooseUsReason3": "write here for a whyChooseUsReason3 ",
      "whyChooseUsReason3ShortDescription": "write here for a whyChooseUsReason3ShortDescription",
  ...
  const sendMessageToChild = () => {
    const iframe = document.getElementById("myIframe");
    console.log("data sent to iframe", gptWebContent);
    iframe.contentWindow.postMessage(gptWebContent, "*");
  ...
  useLayoutEffect(() => {
    ...
    const fetchData = async () => {
      const prompt = JSON.stringify({
        prompt: `Write a content for website with the structure sholud ${JSON.stringify(structure)} i want response sholud json like the structure , for ${businessName} business, and the description is: ${description} slash n`,
  ...
  useEffect(() => {
    const handleMouseEnter = () => {
      const iframe = document.getElementById('myIframe');
      if (iframe) {
        iframe.contentWindow.postMessage({ type: 'scroll', scrollAmount: 20 }, '*');
    ...
    const handleMouseLeave = () => {
      const iframe = document.getElementById('myIframe');
      if (iframe) {
        iframe.contentWindow.postMessage({ type: 'stopScrolling', scrollAmount: 20 }, '*');
    ...
    <div>
      <div className=''>
        <div className='space-y-5 px-10 lg:px-16 mt-12'>
          <h1 className='text-3xl font-bold leading-9'> Choose the structure for your website</h1>
          <p className="text-base font-normal leading-6 text-app-text">Select your preferred structure for your website from the options below.</p>
        </div>
        <div className='pb-2 px-10 lg:px-16 xl:pr-36 xl:pl-24 pt-12 custom-confirmation-modal-scrollbar relative'>
          <div className='flex flex-row flex-wrap items-start gap-10 mb-10 select-none'>
            {webSiteList.map((list, index) => (
              <div
                key={index}
                className={`w-[calc(50%_-_20px)] border-app-border-hover cursor-pointer p-1 select-none ${activeIndex === index ? 'border-2 border-plantinate-500 rounded-lg bg-palatinate-blue-100' : 'border'
                  }`}
                ...
                <div className='w-full h-fit p-1 select-none ' id="your-div-id" >
                  <div className=" border border-app-border-hover p-1 select-none">
                    ...
                    <div className="w-full  max-h-[calc(19_/_15_*_100%)] pt-[calc(19_/_15_*_100%)] select-none relative shadow-md overflow-hidden origin-top-left bg-neutral-300">
                      {temLoader && (
                        <button type="button" class="top-1/2 absolute right-1/2 flex" disabled>
                      ...
                      <iframe
                        ...
                        id="myIframe"
                        title="Child iFrame"
                        className={`scale-[0.33] w-[1200px] h-[1600px] absolute left-0 top-0 origin-top-left select-none ${temLoader ? 'opacity-0 ' : 'opacity-100'}`}
                        src={list.link}
                      />


                      <div className="absolute bottom-0 w-full h-14 flex items-center justify-between bg-white px-5 shadow-template-info">
                        <div className="zw-base-semibold text-app-heading capitalize">{list.option}</div>
                        {activeIndex === index && <CheckCircleIcon className="text-palatinate-blue-600" />} {/* Display tick mark if activeIndex matches */}
                      </div>
            ...
            <div className='flex flex-row xs:flex-row xs:items-center items-start gap-x-10 gap-y-10 xs:gap-y-0 flex-wrap'>
              <button onClick={nextStep} className='hover:opacity-70  min-w-[142px]  inline-flex items-center justify-center    bg-palatinate-blue-600 hover:bg-accent-hover  text-white pl-[25px] pr-[23px] py-[13px]  h-[50px]  rounded-md zw-btn-base shadow-sm  focus:outline-none disabled:opacity-25 transition ease-in-out duration-150'>
                <div className='flex justify-center items-center gap-x-2'>
                  <div>Continue</div>
                  {Loader && (
                    <button type="button" class="bg-palatinate-blue-600 " disabled>
